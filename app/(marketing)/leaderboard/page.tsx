      ...
      <main className="mx-auto w-full max-w-5xl px-4 py-10 md:py-16">
        <header className="mb-8 flex flex-col gap-3 md:mb-10">
          ...
          <h1 className="font-display text-balance text-3xl font-bold leading-tight tracking-tight text-foreground md:text-5xl">
            Top 20 ReLoopers{" "}
            {activeSchoolName ? (
              <span className="text-brand-green-deep">· {activeSchoolName}</span>
            ) : (
              <span className="text-brand-green-deep">toàn quốc</span>
            )}
          </h1>
          <p className="max-w-2xl text-pretty text-base leading-relaxed text-muted-foreground">
            Mỗi lần scan, đăng listing, hoàn tất trao đổi đều cộng eco-points. Đua
            cùng bạn cùng trường — top 20 cập nhật mỗi 60 giây.
          </p>
        </header>

        <div className="mb-8 flex flex-col items-start gap-4 rounded-xl border border-foreground/10 bg-eco-bg-soft px-4 py-4 md:flex-row md:items-center md:justify-between md:px-5">
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed bg-muted/30 px-6 py-16 text-center">
      ...
      <p className="mt-2 max-w-md text-sm text-muted-foreground">
        Hãy là người đầu tiên scan đồ và đăng listing — eco-points đầu tiên đang
        chờ bạn.
      </p>
    </div>
  )
